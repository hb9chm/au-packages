...
function AddLibOVersionsToStreams($streams, $branch, $from, $to) {
  ...
  foreach ($row in $versions) {
    ...
    if ($branch -eq "still") {
      $package.PackageName = "libreoffice-sdk-still"
      $package.Title = "LibreOffice SDK Still"
    } else {
      $package.PackageName = "libreoffice-sdk-fresh"
      $package.Title = "LibreOffice SDK Fresh"
    }
